...
void ListFilesInDirectory(
    HWND hwnd, const std::wstring& directory, std::wstring& EditText, std::wstring Indent
)
{
    WIN32_FIND_DATA ffd;
    ...
    {
        DisplayErrorBox(
            hwnd, L"FindFirstFile() failed, please provide valid directory.", GetLastError()
        );
        return;
    }
    {
            ...
            if (wcscmp(ffd.cFileName, L".") != 0 && wcscmp(ffd.cFileName, L"..") != 0)
            {
                if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
                {
                    ...
                    ListFilesInDirectory(
                        hwnd, directory + L"\\" + ffd.cFileName, EditText, Indent + L"|   "
                    );
                }
                else
                {
    ...
    {
        DisplayErrorBox(hwnd, L"FindFirstFile() failed", dwError);
    }
    FindClose(hFind);
}
